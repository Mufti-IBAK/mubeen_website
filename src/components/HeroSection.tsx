...
export const HeroSection = () => {
  ...
  useGSAP(
    () => {
      ...
      const bg = container.current?.querySelector(
        ".hero-animated-bg"
      ) as HTMLElement | null;
      if (bg) {
        gsap.set(bg, {
  ...
  return (
    <section
      ref={container}
      className="relative isolate min-h-[75vh] w-full overflow-hidden"
    >
      ...
      <div className="w-full text-center py-16 md:py-20">
        ...
        <p
          ref={subtitleRef}
          className="mt-5 text-lg md:text-2xl text-[hsl(var(--muted-foreground))] max-w-3xl mx-auto"
        >
          Authentic Islamic education with a modern experience — empowering
          students with knowledge, character, and action.
        </p>
        <div
          ref={ctasRef}
        ...
        >
          ...
          {isAuthed ? (
            isAdmin ? (
              <Link href="/admin" className="btn-outline px-8 py-3 text-base">
                Admin
              </Link>
            ) : (
              <Link

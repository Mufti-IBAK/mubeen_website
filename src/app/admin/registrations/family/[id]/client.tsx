...
export default function FamilyDetailClient({ familyHeadId }: { familyHeadId: string }) {
  ...
  const loadFamilyData = async () => {
    ...
    try {
      ...
      const allEnrollments = [headData, ...(membersData || [])];
  ...
  const updatePaymentStatus = async (enrollmentId: number, status: string) => {
    try {
      ...
      if (familyHead?.id === enrollmentId) {
        setFamilyHead(prev => prev ? { ...prev, payment_status: status } : null);
      }
      setFamilyMembers(prev =>
        prev.map(member =>
          member.id === enrollmentId ? { ...member, payment_status: status } : member
        )
      );
    } catch (error) {
  ...
  const formatFormData = (formData: Record<string, unknown> | null) => {
    ...
    return Object.entries(formData)
      .filter(([key, value]) => !key.includes('section') && value !== '' && value !== null)
      .map(([key, value]) => ({
  ...
  if (!familyHead) {
    return (
      <div className="space-y-6">
        ...
        <div className="py-12 text-center">
          <p className="text-gray-600">The requested family registration could not be found.</p>
        </div>
      </div>
    );
  }
  ...
  const allMembers = familyMembers.filter(member => member.registration_type !== 'family_head');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/admin/registrations" className="flex items-center gap-2 btn-outline">
            <FiArrowLeft className="w-4 h-4" />
            Back to Registrations
          </Link>
          <div>
            ...
            <p className="text-muted-foreground">{program?.title || 'Unknown Program'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 border border-blue-200 bg-blue-50 rounded-xl">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-blue-700">Payment responsible & primary contact</p>
            </div>
          </div>
          <span className={`badge ${
            familyHead.payment_status === 'paid' ? 'bg-green-100 text-green-700' : 
            familyHead.payment_status === 'refunded' ? 'bg-yellow-100 text-yellow-700' : 
        ...
        <div className="grid grid-cols-1 gap-4 mb-6 md:grid-cols-2">
          ...
          <div className="text-sm text-gray-600">
            <p><span className="font-medium text-gray-900">Registered:</span> {new Date(familyHead.created_at).toLocaleString()}</p>
            <p><span className="font-medium text-gray-900">Family Size:</span> {familyHead.family_size || 'Unknown'} members</p>
            <p><span className="font-medium text-gray-900">Total Amount:</span> {familyHead.currency || 'NGN'} {familyHead.amount?.toLocaleString() || '-'}</p>
            {familyHead.transaction_id && (
              <p><span className="font-medium text-gray-900">Transaction ID:</span> {familyHead.transaction_id}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          <button 
            onClick={() => updatePaymentStatus(familyHead.id, familyHead.payment_status === 'paid' ? 'pending' : 'paid')}
        ...
        {familyHead.form_data && (
          <details className="mt-4">
            <summary className="mb-2 text-sm font-medium text-blue-900 cursor-pointer">View Family Head Information</summary>
            <div className="grid grid-cols-1 gap-4 p-4 bg-white rounded-lg md:grid-cols-2">
              {formatFormData(familyHead.form_data)?.map(({ label, value }, index) => (
      ...
      <div>
        ...
        {allMembers.length === 0 ? (
          <div className="p-8 text-center rounded-lg bg-gray-50">
            <p className="text-gray-600">No individual family member registrations found.</p>
            <p className="mt-1 text-sm text-gray-500">
              This may be a family registration that only collected the family head information.
            </p>
          </div>
        ) : (
              ...
              return (
                <div key={member.id} className="p-4 bg-white border border-gray-200 rounded-lg">
                  ...
                  <div className="space-y-2 text-sm">
                    <p><span className="font-medium">Status:</span> Covered by family payment</p>
                    <p><span className="font-medium">Registered:</span> {new Date(member.created_at).toLocaleDateString()}</p>
                  </div>

                  {member.form_data && (
                    <details className="mt-3">
                      <summary className="text-xs font-medium text-blue-600 cursor-pointer">View Details</summary>
                      <div className="mt-2 space-y-2">
                        {formatFormData(member.form_data)?.slice(0, 5).map(({ label, value }, index) => (
                          <div key={index}>

...
export default function Client({ idParam }: { idParam: string }) {
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/admin/registrations/by-user?${params.toString()}`,
    ...
    rows.forEach((r) => {
      lines.push(
        [
          ...
          r.type,
          r.program_id ?? "",
          JSON.stringify(r.program_title || ""),
          r.amount ?? "",
          r.currency || "",
  ...
  const downloadCsv = () => {
    const base = (rows[0]?.user_name || rows[0]?.user_email || "user")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "_")
      .replace(/^_+|_+$/g, "");
  ...
  const removeAll = async () => {
    if (!confirm("Delete ALL records for this user? This cannot be undone."))
      return;
    await fetch(
      `/api/admin/success-enroll/user/${encodeURIComponent(idParam)}/delete`,
      { method: "DELETE" }
    );
    setRows([]);
  };
  ...
  const saveEditor = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/success-enroll/${editing.id}/update`,
        {
          method: "PATCH",
      ...
      setRows((prev) =>
        prev.map((r) =>
          r.id === editing.id ? { ...r, ...({ form_data: parsed } as any) } : r
        )
      );
      setEditing(null);
      ...
      toast({
        ...
        description:
          linkInput.trim() === ""
            ...
            : "Classroom link saved successfully",
      });
    } catch (e: any) {
      toast({
        title: "Error",
    ...
    <>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="btn-destructive" onClick={removeAll}>
              Delete All
            </button>
          </div>
        </div>
          ...
          <div className="card overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead>
                ...
                <th className="py-2 pr-4">Program/Skill</th>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b">
                    ...
                    <td className="py-2 pr-4">
                      {r.program_id
                        ? r.program_title || `Prog: ${r.program_id}`
                        : (r as any).skill_id
                        ? `Skill: ${(r as any).skill_id}`
                        : "Other"}
                    </td>
                    <td className="py-2 pr-4">
                      {r.currency || "NGN"}{" "}
                    ...
                    <td className="py-2 pr-4">
                      <div className="flex gap-2">
                        <button
                          ...
                          aria-label={`View and edit form for enrollment ${r.id}`}
                        >
                          Form
                        </button>
                        <button
                          className="btn-primary"
                          onClick={() => openLinkEditor(r)}
                          aria-label={`Add or edit classroom link for enrollment ${r.id}`}
                        >
                          {classLinks[r.id] ? "Link" : "Add Link"}
                        </button>
                        <button
                          className="btn-destructive"
                          onClick={() => remove(r.id)}
                          aria-label={`Delete enrollment ${r.id}`}
                        >
                          Del
                        </button>
                      </div>
      ...
      {editing && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Edit enrollment form"
        >
          <div
        ...
        >
          ...
          <div className="relative z-10 w-full max-w-lg card">
            <div className="card-body space-y-4">
              <div>
                <h2 id="link-modal-title" className="text-lg font-semibold">
                  Classroom Link
                </h2>
                <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
                  {linkEditing.program_title ||
                    `Program ${linkEditing.program_id}`}{" "}
                  - Enrollment #{linkEditing.id}
                </p>
              </div>
              <div>
                <label
                  htmlFor="classroom-link-input"
                ...
                >
                  Leave empty to remove the link. Students will see a &quot;Join
                  Classroom&quot; button if a link is set.
                </p>
              </div>
              <div className="flex items-center justify-end gap-2">
                <button

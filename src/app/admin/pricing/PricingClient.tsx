...
export function PricingClient() {
  ...
  const load = async () => {
    ...
    // Fetch all skills
    const { data: skills } = await supabase.from('skills').select('id, title').order('title');
    ...
    <div className="card">
      <div className="card-body overflow-x-auto">
        ...
        <table className="table-auto w-full text-left">
          ...
          <tbody>
            {items.map((item, idx) => (
              <tr key={`${item.entity_type}-${item.entity_id}`} className="border-b hover:bg-muted/50">
                <td className="p-2">
                  <span className={`badge ${item.entity_type === 'program' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>
                    {item.entity_type}
                  </span>
                </td>
            ...
            {items.length === 0 && <tr><td colSpan={5} className="p-4 text-center">No programs or skills found.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>

...
export default function AdminProgramsNewPage() {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSaving(false);
      if (!res.ok || !json?.ok) {
        setMessage(json?.error || "Failed to create skill");
        return;
      }
      setMessage("Skill created! Redirecting to configuration...");
      window.location.href = `/admin/skills/${json.id}`;
    } catch (err: any) {
      setSaving(false);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold text-brand-dark font-heading">
          New Program
        </h1>
      ...
      >
        ...
        <div className="md:col-span-2">
          ...
          <div className="space-y-3">
            ...
            {instructorsArr.length === 0 && (
              <p className="text-xs text-brand-dark/70">No instructors yet.</p>
            )}
          </div>
        </div>
        ...
        <div className="md:col-span-2">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              type="button"
              className="btn-outline"
              onClick={() => setFaqsArr((prev) => [...prev, { q: "", a: "" }])}
            >
              Add FAQ
            </button>
          </div>
          <div className="space-y-3">
            ...
            {faqsArr.length === 0 && (
              <p className="text-xs text-brand-dark/70">No FAQs yet.</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          />
          {uploadMsg && (
            <p className="text-xs text-brand-dark/70 mt-1">{uploadMsg}</p>
          )}
        </div>
        <div>
        ...
        <div className="md:col-span-2">
          <button
            type="submit"
            disabled={saving}
            className="rounded-md bg-brand-primary px-4 py-2 text-white font-semibold"
          >
            {saving ? "Saving..." : "Save Program"}
          </button>
          {message && (
            <p className="text-sm text-brand-dark/70 mt-2">{message}</p>
          )}
        </div>
      </form>
    </div>

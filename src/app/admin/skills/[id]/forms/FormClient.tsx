  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/forms/upsert', {
        method: 'POST', headers: { 'content-type': 'application/json', ...(token ? { 'authorization': `Bearer ${token}` } : {}) }, body: JSON.stringify({ scope: 'skill', id: skillId, form_type: 'individual', schema })
      });
    ...
    <div className="space-y-4">
      <div className="card">
        <div className="card-body">
          ...
          <p className="text-sm text-[hsl(var(--muted-foreground))]">Create and manage the registration forms for this skill.</p>
        </div>
      </div>

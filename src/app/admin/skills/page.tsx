...
export default function AdminSkillsListPage() {
  ...
  const remove = async (id: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/skills/${id}/delete`, {
        method: 'DELETE',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-extrabold tracking-tight bg-gradient-to-r from-[hsl(var(--primary))] to-[hsl(var(--accent))] bg-clip-text text-transparent">Skills</h1>
        <Link href="/admin/skills/new" className="btn-primary">New Skill</Link>
      </div>

      {loading ? (
        <div className="card"><div className="card-body"><p className="text-[hsl(var(--muted-foreground))]">Loading...</p></div></div>
      ) : items.length === 0 ? (
        <div className="card"><div className="card-body"><p className="text-[hsl(var(--muted-foreground))]">No skills yet. Click the button to add one.</p></div></div>
      ) : (
            ...
            <div key={p.id} className="prog-card rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm overflow-hidden hover:shadow-md hover-lift transition">
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="pt-2 flex items-center gap-2">
                  <Link href={`/admin/skills/${p.id}`} className="btn-outline px-3 py-1">Edit</Link>
                  <Link href={`/admin/skills/${p.id}/forms`} className="btn-outline px-3 py-1">Forms</Link>

...
export function CourseFormsClient({ courseId }: { courseId: number }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/forms/upsert', {
        method: 'POST', headers: { 'content-type': 'application/json', ...(token ? { 'authorization': `Bearer ${token}` } : {}) }, body: JSON.stringify({ scope: 'course', id: courseId, form_type: active, schema })
      });
    ...
    <div className="space-y-4">
      <div className="card">
        <div className="card-body">
          <h2 className="text-xl font-bold mb-1">Course Form Builder</h2>
          <p className="text-sm text-[hsl(var(--muted-foreground))]">Create and manage the registration forms for this course.</p>
        </div>
      </div>

...
export async function GET(req: NextRequest) {
  try {
    ...
    const programId = Number(search.get('program_id'));
    ...
    const [{ data: enr }, { data: drafts }] = await Promise.all([
      admin.from('enrollments').select('id,user_id,program_id,status,payment_status,amount,currency,transaction_id,form_data,classroom_link,defer_active,completed_at,created_at').eq('program_id', programId).order('created_at', { ascending: false }),
      admin.from('registration_drafts').select('id,user_id,program_id,registration_type,family_size,last_edited_at').eq('program_id', programId).order('last_edited_at', { ascending: false })
    ]);
    ...
    console.error('by-program error', e);

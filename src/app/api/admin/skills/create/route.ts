  ...
  try {
    ...
    const payload: Record<string, unknown> = {
      ...
      outcomes: Array.isArray(body.outcomes)
        ? body.outcomes
        : typeof body.outcomes === "string" && body.outcomes
    ...
    // After creating a skill, auto-seed default Individual form from existing template
    try {
      const url = getEnv("NEXT_PUBLIC_SUPABASE_URL");
      const service =
        process.env.SUPABASE_SERVICE_ROLE_KEY ||
        process.env.SUPABASE_SERVICE_KEY;
      // Fetch a default individual form schema from any existing program (or skill if available, but programs is safer source of truth for now)
      const tplRes = await fetch(
        `${url}/rest/v1/program_forms?form_type=eq.individual&select=schema&limit=1`,
        {
          headers: {
            apikey: service as string,
            Authorization: `Bearer ${service}`,
      ...
      if (!schema) {
        schema = {
          title: `Register for ${rawTitle || "Skill"}`,
          fields: [
            {
              type: "text",
              name: "full_name",
      ...
      // Insert for new skill
      // Using 'skill_forms' table
      await fetch(`${url}/rest/v1/skill_forms`, {
        method: "POST",
        headers: {
          apikey: service as string,
          Authorization: `Bearer ${service}`,
      ...
      // Using 'skill_plans' table
      // Removed duration_months as per request
      await fetch(`${url}/rest/v1/skill_plans`, {
        method: "POST",
        headers: {
          apikey: service as string,
          Authorization: `Bearer ${service}`,

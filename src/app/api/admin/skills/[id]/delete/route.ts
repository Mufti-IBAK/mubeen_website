...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!skillId || Number.isNaN(skillId)) return NextResponse.json({ ok: false, error: "Invalid id" }, { status: 400 });
    ...
    const { data: row } = await supabaseAdmin.from('skills').select('slug').eq('id', skillId).single();
    const { error } = await supabaseAdmin.from("skills").delete().eq("id", skillId);
    if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 400 });

    // Revalidate pages
    revalidatePath('/skill-up');
    revalidatePath('/admin/skills');
    if (row?.slug) revalidatePath(`/skill-up/${row.slug}`);

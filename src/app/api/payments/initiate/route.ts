...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!emailRegex.test(email)) {
      console.warn(
        "Invalid email provided for payment, attempting fallback to session user email",
        { email }
      );
    }

    const kind = required("kind", body.kind ?? null) as
      | "program"
      | "donation"
      | "other"
      | "skill";
    ...
    // If client passes an existing enrollment row, update it; else create one
    const incomingId =
      body.enrollment_id || body.success_enroll_id
        ? Number(body.enrollment_id || body.success_enroll_id)
        : null;
    let id: number;
    if (incomingId) {
      ...
      // Generate tx_ref BEFORE creating enrollment to avoid race conditions
      const tempId = Date.now(); // Temporary ID for tx_ref generation
      const tx_ref_temp = `er-temp-${tempId}`;
      ...
      const { data: inserted, error: insErr } = await admin
        ...
        .insert({
          ...
          tx_ref: tx_ref_temp, // Include tx_ref in initial insert
        })
        .select("id")
        .single();
      if (insErr)
      ...
      // Update with proper tx_ref now that we have the real ID
      const tx_ref_final = `er-${id}-${Date.now()}`;
      await admin
        .from("enrollments")
        .update({ tx_ref: tx_ref_final })
        .eq("id", id);
    }
    ...
    if (
      ...
      (kind === "skill" && (body.skill_id || body.meta?.skill_id))
    ) {
      const eid =
        kind === "program"
          ? program_id
          : Number(body.skill_id || body.meta?.skill_id);
      ...
      if (plan) {
        ...
        // Persist authoritative amount in enrollments
        await admin
          .from("enrollments")
        ...
        const { data: seRow } = await admin
          ...
          .eq("id", id)
          .maybeSingle();
        const cachedAmount = (seRow as any)?.amount as
          | number
          | null
          | undefined;
        if (cachedAmount && Number(cachedAmount) > 0) {
          ...
          currency =
            ((seRow as any)?.currency as string | null) || currency || "NGN";
        } else {
          console.error("Payment missing unified pricing configuration", {
            kind,
            eid,
            enrollment_id: id,
          });
    ...
    const tx_ref = `er-${id}-${Date.now()}`;
    await admin.from("enrollments").update({ tx_ref }).eq("id", id);

    const FLW_SK = required(
      "FLUTTERWAVE_SECRET_KEY",
      process.env.FLUTTERWAVE_SECRET_KEY ?? null
    );
    ...
    const site =
      ...
      /^https?:\/\//i.test(process.env.NEXT_PUBLIC_SITE_URL)
        ...
        : origin;
    const redirect_url = `${site}/payment-success?ref=${encodeURIComponent(
      tx_ref
    )}`;
    ...
    const payload = {
      ...
      meta: { 
        success_enroll_id: id,  // CRITICAL: Webhook uses this to identify enrollment
        enrollment_id: id, 
        program_id, 
        kind 
      },
    ...
    if (!res.ok || !link)
      return NextResponse.json(
        { ok: false, error: "failed_to_create_payment", details: json },
        { status: 500 }
      );

    // Return a small HTML page that immediately redirects; more reliable than HTTP redirect across all clients
    const html = `<!doctype html>
<html><head><meta http-equiv="refresh" content="0;url=${link}"><script>location.replace(${JSON.stringify(
      link
    )})</script></head>

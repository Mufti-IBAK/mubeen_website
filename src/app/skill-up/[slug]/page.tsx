          ...
          <div className="p-6 md:p-10">
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
              <div className="lg:col-span-2">
                <h2 className="mb-4 text-2xl font-bold text-brand-dark font-heading">
                  Skill Overview
                </h2>
                {program.overview ? (
                  <div
                    className="prose max-w-none prose-emerald prose-lg text-brand-dark/80 prose-headings:font-heading prose-headings:font-bold prose-p:leading-relaxed prose-li:marker:text-brand-primary [&>ul]:list-disc [&>ul]:pl-5 [&>ol]:list-decimal [&>ol]:pl-5"
                ...
                {program.prerequisites && (
                  <div className="mt-4">
                    <h3 className="mb-2 text-lg font-semibold text-brand-dark font-heading">
                      Prerequisites
                    </h3>
                    <div
                      className="prose max-w-none prose-emerald text-brand-dark/80"
                      dangerouslySetInnerHTML={{
                        __html: program.prerequisites,
                      }}
                    />
                  </div>
                )}
                {program.outcomes && program.outcomes.length > 0 && (
                  <div className="mt-6">
                    <h3 className="mb-3 text-lg font-semibold text-brand-dark font-heading">
                      Learning Outcomes
                    </h3>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {tags.map((tag: string) => (
                      <span
                        key={tag}
                        className="px-3 py-1 text-sm font-medium rounded-full bg-brand-primary/10 text-brand-primary"
                      >
                        {tag}
                      </span>
                    ))}
              ...
              <div className="p-6 rounded-lg bg-brand-bg">
                <h3 className="mb-4 text-lg font-semibold text-brand-dark font-heading">
                  Key Information
                </h3>
            ...
            {/* Instructors */}
            {Array.isArray((program as any).instructors) &&
              (program as any).instructors.length > 0 && (
                <div className="mt-12">
                  <h2 className="mb-4 text-2xl font-bold text-brand-dark font-heading">
                    Meet the Instructors
                  </h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {(program as any).instructors.map(
                      (ins: any, idx: number) => (
                        <div
                          key={idx}
                          className="rounded-xl border border-[hsl(var(--border))] bg-white shadow-sm p-4 flex items-center gap-4"
                        >
            ...
            {/* FAQs */}
            {Array.isArray((program as any).faqs) &&
              (program as any).faqs.length > 0 && (
                <div className="mt-12">
                  <h2 className="mb-4 text-2xl font-bold text-brand-dark font-heading">
                  ...
                  <div className="space-y-3">
                    {(program as any).faqs.map((f: any, idx: number) => (
                      <details
                        key={idx}
                        className="group rounded-lg border border-[hsl(var(--border))] bg-white p-4"
                      >
            ...
            {/* Schedule timeline */}
            {(((program as any).schedule?.items &&
              Array.isArray((program as any).schedule.items)) ||
              Array.isArray((program as any).schedule)) && (
              <div className="mt-12">
                <h2 className="mb-4 text-2xl font-bold text-brand-dark font-heading">
                  Schedule
                </h2>
                <ol className="relative border-l border-[hsl(var(--border))] ml-3">
                  {(
                    (Array.isArray((program as any).schedule?.items)
                      ? (program as any).schedule.items
                      : (program as any).schedule) as any[]
                  ).map((s: any, idx: number) => (
                    <li key={idx} className="mb-6 ml-4">
                      <div className="absolute w-3 h-3 bg-[hsl(var(--primary))] rounded-full -left-1.5 border border-white" />
                      <time className="mb-1 text-sm font-normal leading-none text-brand-dark/60">
                        {s.when || s.date}
                      </time>
                      ...
                      {s.description && (
                        <p className="text-brand-dark/80">{s.description}</p>
                      )}
                    </li>
                  ))}
                </ol>
